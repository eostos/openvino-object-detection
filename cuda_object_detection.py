...
def loop_and_detect(cam, trt_yolo, conf_th, vis, config,connect_redis,device):
...
pip3 install
    ...
    while True:
        ...
        util.send_video(img,connect_redis,config["device_id"])
        print(len(boxes))
        #if len(boxes) >= 1 :
...
def main():
   ...
   #     cam.img_width, cam.img_height)
    loop_and_detect(None, trt_yolo, args.conf_thresh, vis=vis,url=vid_path,connect_redis=connect_redis,device=device)
