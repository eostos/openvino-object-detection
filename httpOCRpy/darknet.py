...
'''
def prev_detect_image(net, meta, im, thresh=.5, hier_thresh=.5, nms=.45, debug= False):
	#import cv2
	...
	for j in range(num):
		...
		for i in range(meta.classes):
			...
			if dets[j].prob[i] > 0:
				b = dets[j].bbox
				if False:
					print('b.x: ', round(b.x, 2), end=' , ')
					print('b.y: ', round(b.y, 2), end=' , ')
					print('b.w: ', round(b.w, 2), end=' , ')
					print('b.h: ', round(b.h, 2))
					print('im.w: ', im.w, ' - im.h: ', im.h)
				#if b.w<1 and b.h<1:
				#	continue
				if b.w>1 and b.h>1:
					b.x = b.x / im.w
				...
				probs = [0.] * nClasses
				probs[i] = topProb
				elem_i = [b.x, b.y, b.w, b.h, topProb] + probs
				#print('list: ', [round(x, 2) for x in elem_i[:5]])
				res.append(elem_i)
	if debug: print("did range")
	res = sorted(res, key=lambda x: -x[1])
	if debug: print("did sort")
